...
def dashboard():
    ...
    put_buttons(['Start Packet Capture', 'Search Packet', 'Download Report', 'Firewall Automation', 'Advanced Firewall Automation', 'Syslog', 'DNS Server Management'],
                onclick=[start_packet_capture, search_packet, download_report, firewall_automation, advanced_firewall_automation, syslog_tab, dns_server_tab,])
    put_html("</div>")
    put_html("<div class='main-content'>")
    put_markdown('# Dashboard')
    put_markdown(f"Hostname: {socket.gethostname()}")
    put_html("</div>")

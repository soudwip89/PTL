...
# Add the configure_email function to the web UI
def snmp_monitoring():
    clear()
    put_html("""
    <style>
    ...
    # Add a button for email configuration
    put_buttons(["Configure Email"], onclick=[configure_email])
    ...
    for device in devices:
        device_rows.append([
            ...
            put_buttons(["Delete"], onclick=[lambda d=device: delete_device(d)])  # Add Delete button
        ])

    put_table([
        ["Device Name", "IP/Hostname", "Port Number", "Transport", "Pin", "Delete"],
        *device_rows
    ])
...
# Function to monitor devices (dummy function for now)
def monitor_device():
    for device in devices:
        # Dummy check (replace with actual SNMP check)
        if device.get("status") == "down":
            send_email(f"Device Down: {device['name']}", f"The device {device['name']} is down.")
    snmp_monitoring()
...
from pywebio.output import clear, put_buttons, put_html, put_markdown, put_table
from threading import Thread
import time

devices = [
    {"name": "Router1", "ip_hostname": "192.168.1.1", "port": 161, "transport": "UDP"},
    {"name": "Switch1", "ip_hostname": "192.168.1.2", "port": 161, "transport": "UDP"},
]
...
def snmp_monitoring():
    ...
    put_buttons(['SNMP Configuration', 'Email Configuration'], onclick=[snmp_config_page, email_config_page])
    ...
    for device in devices:
        device_rows.append([
            ...
            device["port"],
            device["transport"],
            put_buttons(["Pin"], onclick=[lambda d=device: pin_device(d)])
        ])
    ...
    # Start device status check in a separate thread
    Thread(target=check_device_status).start()


def snmp_config_page():
    clear()
    put_markdown('## SNMP Configuration')

    put_buttons(['SNMPv2 Configuration', 'SNMPv3 Configuration'], onclick=[snmpv2_config_page, snmpv3_config_page])


def snmpv2_config_page():
    clear()
    put_markdown('## SNMPv2 Configuration')

    snmpv2_config = input_group("SNMPv2 Configuration", [
        input("Community String", name="community_string_v2"),
    ])

    monitoring_options = input_group("Monitoring Options", [
        select("Monitor Disk Space", ["Yes", "No"], name="disk_space"),
        select("Monitor Bandwidth Utilization", ["Yes", "No"], name="bandwidth_utilization"),
        select("Monitor Memory Utilization", ["Yes", "No"], name="memory_utilization"),
        select("Monitor System Health", ["Yes", "No"], name="system_health"),
    ])

    # Save SNMPv2 and monitoring configuration details to database or configuration file


def snmpv3_config_page():
    clear()
    put_markdown('## SNMPv3 Configuration')

    snmpv3_config = input_group("SNMPv3 Configuration", [
        input("Username", name="username_v3"),
        input("Authentication Protocol", name="auth_protocol_v3"),
        input("Authentication Password", type="password", name="auth_password_v3"),
        input("Privacy Protocol", name="privacy_protocol_v3"),
        input("Privacy Password", type="password", name="privacy_password_v3"),
    ])
    ...
    # Save SNMPv3 and monitoring configuration details to database or configuration file
    # Save SNMP and monitoring configuration details to database or configuration file

    # Save SNMP configuration details to database or configuration file

    # Add code for SNMP configuration inputs

def email_config_page():
    global smtp_config
    smtp_config = input_group("Email Configuration", [
        input("SMTP Server", name="smtp_server"),
        input("SMTP Port", name="smtp_port"),
        input("Sender Email", name="sender_email"),
        input("Recipient Email", name="recipient_email"),
        input("Email Password", type="password", name="email_password"),
    ])
...
def check_device_status():
    while True:
        for device in devices:
            # Placeholder for actual SNMP check logic
            device_status = True  # Assume the device is up for now
            if not device_status:
                send_email(f"Device Down: {device['name']}", f"The device {device['name']} ({device['ip_hostname']}) is down.")
...
def send_email(subject, body):
    ...
    try:
        ...
        server.starttls()
        server.login(smtp_config["sender_email"], smtp_config["email_password"])
        server.sendmail(smtp_config["sender_email"], smtp_config["recipient_email"], msg.as_string())
        server.close()
...
import subprocess
from pywebio.output import put_html, put_markdown, put_buttons, put_table, put_text
from pywebio.input import input_group, input, PASSWORD, NUMBER
from pywebio import start_server

# Global variables for devices and hostname
devices = [
    {"name": "Device1", "ip_hostname": "192.168.1.1", "port": 22, "transport": "TCP"},
...
def ssh_to_device(device):
    ip = device["ip_hostname"]
    port = device["port"]
    subprocess.Popen(["putty", "-ssh", f"{ip}", "-P", str(port)])

def pin_device(device):
    # Logic to pin the device (e.g., move to top of list)
    pass

def add_device():
    device = input_group("Add Device", [
        input("Device Name", name="name"),
        input("IP/Hostname", name="ip_hostname"),
        input("Port Number", name="port", type=NUMBER),
        input("Transport (TCP/UDP)", name="transport")
    ])
    devices.append(device)
    monitor_device()

def send_email_notification(device):
    if not email_config:
        put_text("Email configuration is missing. Unable to send email.")
        return

    subject = f"Device Offline Alert: {device['name']}"
    body = f"The device {device['name']} ({device['ip_hostname']}) is offline."
...
def send_email(subject, body):
    try:
        ...
        text = msg.as_string()
        server.sendmail(email_config['username'], email_config['recipient_email'], text)
        server.quit()

        put_text("Notification email sent successfully.")
    except Exception as e:
        put_text(f"Failed to send email: {e}")

def monitor_status_periodically():
    for device in devices:
        status, _ = check_reachability(device["ip_hostname"], device["transport"])
        if "Offline" in status:
            send_email_notification(device)
    Timer(60, monitor_status_periodically).start()

def configure_email():
    global email_config
    email_config = input_group("Email Configuration", [
        input("SMTP Server", name="smtp_server"),
        input("SMTP Port", name="smtp_port", type=NUMBER),
        input("Username", name="username"),
        input("Password", name="password", type=PASSWORD),
        input("Recipient Email", name="recipient_email"),
    ])
    put_text("Email configuration updated.")



import os
from pywebio import start_server
from pywebio.output import put_html, put_markdown, put_buttons, clear
...
def upload_logo(logo_file):
    global logo_path  # Make logo_path accessible globally
    # Save the uploaded logo file to the appropriate directory
    filename = os.path.join(UPLOAD_DIR, logo_file['filename'])
    with open(filename, 'wb') as f:
        f.write(logo_file['content'])
    # Update logo_path to the newly uploaded logo file
    logo_path = f"/static/uploads/{logo_file['filename']}"
    # Refresh the dashboard to display the uploaded logo
    dashboard()
logo_path = "/static/default_logo.png"  # Default logo path

def dashboard():
    clear()
    logo_path = "/static/uploads/Safeimagekit-resized-img.png"  # Update with the actual path to your logo
    ...
    if logo_path:
        put_html(f"<div class='logo'><img src='{logo_path}' alt='Logo'></div>")
    put_markdown('## Dashboard Menu')
    put_html("<div class='menu-buttons'>")
    put_buttons(['Monitor Device', 'Settings', 'SNMP Monitoring', 'Authentication', 'Logout', 'Upload Logo','DHCP IP Status'],
                onclick=[monitor_device, settings, snmp_monitoring, authentication, logout, upload_logo, dhcp_ip_status])
    put_buttons(['Start Packet Capture', 'Search Packet', 'Download Report', 'Firewall Automation', 'Syslog', 'IPAM',
                 'DNS Server Management', 'DHCP Configurations'],
                onclick=[start_packet_capture, search_packet, download_report, firewall_automation, syslog_tab, ipam,
                         dns_server_tab, view_dhcp_subnets])
    put_html("</div>")
    put_html("</div>")

    put_html("<div class='main-content'>")
    put_markdown('# Dashboard')
    put_html("<div class='logo'>")
    if logo_path:
        put_html(f"<img src='{logo_path}' alt='Logo'>")
    put_html("</div>")
...
if __name__ == "__main__":
    # Run the PyWebIO server in a separate thread
    Thread(target=run_pywebio).start()
